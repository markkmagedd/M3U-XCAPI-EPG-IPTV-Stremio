...
if (process.env.REDIS_URL) {
  try {
    ...
    console.warn(
      "[REDIS] ioredis not installed or failed, falling back to in-memory LRU",
    );
    redisClient = null;
  }
}
...
function makeLogger(cfgDebug) {
  ...
  return {
    debug: (...a) => {
      if (enabled) console.log("[DEBUG]", ...a);
    },
...
function createCacheKey(config) {
  ...
  return crypto
    .createHash("md5")
    .update(stableStringify(minimal))
    .digest("hex");
}
...
class M3UEPGAddon {
  constructor(config = {}, manifestRef) {
    ...
    if (
      typeof this.config.epgOffsetHours !== "number" ||
      !isFinite(this.config.epgOffsetHours)
    )
  ...
  buildGenresInManifest() {
    ...
    const tvCatalog = this.manifestRef.catalogs.find(
      (c) => c.id === "iptv_channels",
    );
    const movieCatalog = this.manifestRef.catalogs.find(
      (c) => c.id === "iptv_movies",
    );
    const seriesCatalog = this.manifestRef.catalogs.find(
      (c) => c.id === "iptv_series",
    );
  ...
  parseM3U(content) {
    ...
    for (const raw of lines) {
      ...
      if (line.startsWith("#EXTINF:")) {
        ...
        const group = (
          currentItem.attributes["group-title"] || ""
        ).toLowerCase();
  ...
  async ensureSeriesInfo(seriesId) {
    ...
    if (this.seriesInfoCache.has(seriesId))
      return this.seriesInfoCache.get(seriesId);

    try {
      const providerModule = require(
        `./src/js/providers/${this.providerName}Provider.js`,
      );
  ...
  async updateData(force = false) {
    ...
    if (!force && CACHE_ENABLED) {
      ...
      if (
        (this.channels.length || this.movies.length || this.series.length) &&
        now - this.lastUpdate < 900000
      ) {
  ...
  generateMetaPreview(item) {
    ...
    if (item.type === "tv") {
      const epgId =
        item.attributes?.["tvg-id"] || item.attributes?.["tvg-name"];
      ...
      meta.description =
        item.plot || item.attributes?.["plot"] || `Movie: ${item.name}`;
      meta.genres = item.attributes?.["group-title"]
        ? [item.attributes["group-title"]]
        : ["Movie"];
      ...
      meta.description =
        item.plot || item.attributes?.["plot"] || "Series / Show";
      meta.genres = item.category
        ? [item.category]
        : item.attributes?.["group-title"]
          ? [item.attributes["group-title"]]
  ...
  async buildSeriesMeta(seriesItem) {
    const seriesIdRaw =
      seriesItem.series_id || seriesItem.id.replace(/^iptv_series_/, "");
    ...
    return {
      ...
      description:
        seriesItem.plot || seriesItem.attributes?.["plot"] || "Series / Show",
      genres: seriesItem.category
        ? [seriesItem.category]
        : seriesItem.attributes?.["group-title"]
          ? [seriesItem.attributes["group-title"]]
          : ["Series"],
      videos,
    };
  ...
  getDetailedMeta(id) {
    ...
    if (item.type === "tv") {
      ...
      if (current) {
        ...
        const end =
          current.stopTime?.toLocaleTimeString([], {
            hour: "2-digit",
            minute: "2-digit",
          }) || "";
      ...
      return {
        ...
        poster:
          ...
          `https://via.placeholder.com/300x450/CC6600/FFFFFF?text=${encodeURIComponent(item.name)}`,
        description,
        genres: item.attributes?.["group-title"]
          ? [item.attributes["group-title"]]
          : ["Movie"],
        year,
      };
...
async function createAddon(config) {
  const manifest = {
    ...
    name: ADDON_NAME,
    description:
      "IPTV addon (M3U / EPG / Xtream) with encrypted configs, caching & series support (Xtream + Direct)",
  ...
  if (debugFlag) {
    console.log("[DEBUG] createAddon start", {
      cacheKey,
      provider: config.provider,
    ...
    console.log(
      `[ADDON] Cache ${CACHE_ENABLED ? "ENABLED" : "DISABLED"} for config ${cacheKey}`,
    );
  }
  ...
  const buildPromise = (async () => {
    ...
    try {
      ...
      if (
        !addonInstance.lastUpdate ||
        Date.now() - addonInstance.lastUpdate > addonInstance.updateInterval
      ) {
    ...
    builder.defineCatalogHandler(async (args) => {
      ...
      try {
        ...
        // If searching (ignore genre filter if search is present)
        if (extra.search) {
          // Reset filtered to full list for global search behavior
          filtered = items;
          const q = extra.search.toLowerCase();
          
          // FIX: Only apply "fuzzy" Latin cleanup if the query yields alphanumeric chars.
          // Otherwise (e.g. Arabic), rely on standard .includes().
          let cleanQ = q.replace(/[^a-z0-9]/g, "");
          if (cleanQ.length === 0 && q.length > 0) cleanQ = null;

          filtered = filtered.filter((i) => {
            if (!i.name) return false;
            const itemLower = i.name.toLowerCase();
            
            // 1. Standard exact substring match (works for all languages including Arabic)
            if (itemLower.includes(q)) return true;
            
            // 2. Fuzzy check (only if we have a valid Latin clean query)
            if (cleanQ) {
              const cleanName = itemLower.replace(/[^a-z0-9]/g, "");
              if (cleanName.includes(cleanQ)) return true;
            }

            // 3. Check category/group-title as backup for series
            if (args.type === "series" && i.category) {
              const catLower = i.category.toLowerCase();
              if (catLower.includes(q)) return true;
              if (cleanQ && catLower.replace(/[^a-z0-9]/g, "").includes(cleanQ))
                return true;
            }
            return false;
          });
        ...
        const metas = filtered
          .slice(skip, skip + 100)
          .map((i) => addonInstance.generateMetaPreview(i));
    ...
    builder.defineMetaHandler(async ({ type, id }) => {
      try {
        if (addonInstance.config.debug) {
          console.log("[DEBUG] Meta handler called", {
            type,
            id,
            seriesCount: addonInstance.series.length,
          });
        }
        if (type === "series" || id.startsWith("iptv_series_")) {
          ...
          if (!meta) {
            ...
            if (seriesItem) {
              ...
              if (addonInstance.config.debug) {
                console.log("[DEBUG] Series meta fallback used", {
                  id,
                  name: seriesItem.name,
                });
              }
